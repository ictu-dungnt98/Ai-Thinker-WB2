...
static axk_err_t axk_mqtt_set_cert_key_data(axk_transport_handle_t ssl, enum axk_mqtt_ssl_cert_key_api what, const char *cert_key_data, int cert_key_len)
{
    ...
    if (len == 0) {
        ...
        blog_error( "Explicit cert-/key-len is not available in SDK version %s", SDK_VER);
...
#else
        blog_error( "Certificate bundle is not enabled for mbedTLS in menuconfig");
        goto axk_mqtt_set_transport_failed;
#endif /* CONFIG_MBEDTLS_CERTIFICATE_BUNDLE */
#else
        blog_error( "Certificate bundle feature is not available in SDK version %s", SDK_VER);
        goto axk_mqtt_set_transport_failed;
...
#else
        blog_error( "Secure element not enabled for axk-tls in menuconfig");
        goto axk_mqtt_set_transport_failed;
#endif /* CONFIG_AXK_TLS_USE_SECURE_ELEMENT */
#else
        blog_error( "Secure element feature is not available in SDK version %s", SDK_VER);
        goto axk_mqtt_set_transport_failed;
...
#else
        blog_error( "Digital Signature not enabled for axk-tls in menuconfig");
        goto axk_mqtt_set_transport_failed;
#endif /* CONFIG_AXK_TLS_USE_DS_PERIPHERAL */
#else
        blog_error( "Digital Signature feature is not available in SDK version %s", SDK_VER);
        goto axk_mqtt_set_transport_failed;
#endif
    }
...
#else
        blog_error( "Password protected keys are not available in SDK version %s", SDK_VER);
        goto axk_mqtt_set_transport_failed;
#endif
    }
...
#else
        blog_error( "PSK authentication configured but not enabled in menuconfig: Please enable AXK_TLS_PSK_VERIFICATION option");
        goto axk_mqtt_set_transport_failed;
#endif
#else
        blog_error( "PSK authentication is not available in SDK version %s", SDK_VER);
        goto axk_mqtt_set_transport_failed;
#endif
    }
...
#else
        blog_error( "APLN configured but not enabled in menuconfig: Please enable MBEDTLS_SSL_ALPN or WOLFSSL_HAVE_ALPN option");
        goto axk_mqtt_set_transport_failed;
#endif
#else
        blog_error( "APLN is not available in SDK version %s", SDK_VER);
        goto axk_mqtt_set_transport_failed;
#endif
    }
...
#else
        blog_error( "Skip certificate common name check is not available in SDK version %s", SDK_VER);
        goto axk_mqtt_set_transport_failed;
#endif
    }
...
static axk_err_t axk_mqtt_check_cfg_conflict(const mqtt_config_storage_t *cfg, const axk_mqtt_client_config_t *user_cfg)
{
    ...
    if (cfg->uri && user_cfg->transport) {
        blog_warn( "Transport config set, but overridden by scheme from URI: transport = %d, uri scheme = %s", user_cfg->transport, cfg->scheme);
        ret = AXK_ERR_INVALID_ARG;
    }
...
axk_err_t axk_mqtt_set_config(axk_mqtt_client_handle_t client, const axk_mqtt_client_config_t *config)
{
    ...
    if (!config->set_null_client_id) {
        ...
        blog_debug( "MQTT client_id=%s", client->connect_info.client_id);
    }
    ...
    if (config->disable_clean_session == client->connect_info.clean_session) {
        ...
        if (!client->connect_info.clean_session && config->set_null_client_id) {
            blog_error( "Clean Session flag must be true if client has a null id");
        }
    }
    if (config->keepalive) {
...
static inline axk_err_t axk_mqtt_write(axk_mqtt_client_handle_t client)
{
    ...
    while (len > 0) {
        ...
        if (wlen < 0) {
            blog_error( "Writing failed: errno=%d", errno);
            return AXK_FAIL;
        } else if (wlen == 0) {
            blog_error( "Writing didn't complete in specified timeout: errno=%d", errno);
...
static axk_err_t axk_mqtt_connect(axk_mqtt_client_handle_t client, int timeout_ms)
{
    ...
    if (client->mqtt_state.outbound_message->length == 0) {
        blog_error( "Connect message cannot be created");
        return AXK_FAIL;
    }
    ...
    blog_debug( "Sending MQTT CONNECT message, type: %d, id: %04X",
    ...
    if (read_len <= 0) {
        blog_error( "%s: mqtt_message_receive() returned %d", __func__, read_len);
        return AXK_FAIL;
    }

    if (mqtt_get_type(client->mqtt_state.in_buffer) != MQTT_MSG_TYPE_CONNACK) {
        blog_error( "Invalid MSG_TYPE raxkonse: %d, read_len: %d", mqtt_get_type(client->mqtt_state.in_buffer), read_len);
        return AXK_FAIL;
    }
    client->mqtt_state.in_buffer_read_len = 0;
    ...
    case MQTT_CONNECTION_REFUSE_PROTOCOL:
        blog_warn( "Connection refused, bad protocol");
        break;
    case MQTT_CONNECTION_REFUSE_SERVER_UNAVAILABLE:
        blog_warn( "Connection refused, server unavailable");
        break;
    case MQTT_CONNECTION_REFUSE_BAD_USERNAME:
        blog_warn( "Connection refused, bad username or password");
        break;
    case MQTT_CONNECTION_REFUSE_NOT_AUTHORIZED:
        blog_warn( "Connection refused, not authorized");
        break;
    default:
        blog_warn( "Connection refused, Unknow reason");
        break;
    }
...
static void axk_mqtt_abort_connection(axk_mqtt_client_handle_t client)
{
    ...
    blog_debug( "Reconnect after %d ms", client->wait_timeout_ms);
...
axk_err_t axk_mqtt_client_set_uri(axk_mqtt_client_handle_t client, const char *uri)
{
    ...
    if (parser_status != 0) {
        blog_error( "Error parse uri = %s", uri);
        return AXK_FAIL;
    }
...
static axk_err_t deliver_publish(axk_mqtt_client_handle_t client)
{
    ...
    if (msg_topic == NULL) {
        blog_error( "%s: mqtt_get_publish_topic() failed", __func__);
        return AXK_FAIL;
    }
    blog_debug( "%s: msg_topic_len=%zu", __func__, msg_topic_len);

    // get payload
    msg_data = mqtt_get_publish_data(msg_buf, &msg_data_len);
    if (msg_data_len > 0 && msg_data == NULL) {
        blog_error( "%s: mqtt_get_publish_data() failed", __func__);
        return AXK_FAIL;
    }
...
post_data_event:
    blog_debug( "Get data len= %zu, topic len=%zu, total_data: %d offset: %zu", msg_data_len, msg_topic_len,
    ...
    if (msg_read_len < msg_total_len) {
        ...
        msg_data_len = axk_transport_read(client-> transport, (char *)client->mqtt_state.in_buffer,
                                          ...
                                          client->config->network_timeout_ms);
        if (msg_data_len <= 0) {
            blog_error( "Read error or timeout: len_read=%zu, errno=%d", msg_data_len, errno);
            return AXK_FAIL;
        }
...
static axk_err_t deliver_suback(axk_mqtt_client_handle_t client)
{
    ...
    if (msg_data_len <= 0) {
        blog_error( "Failed to acquire suback data");
        return AXK_FAIL;
    }
    // post data event
    client->event.data_len = msg_data_len;
...
{
    blog_debug( "pending_id=%d, pending_msg_count = %d", client->mqtt_state.pending_msg_id, client->mqtt_state.pending_msg_count);
...
{
    blog_debug( "mqtt_enqueue_oversized id: %d, type=%d successful",
...
{
    blog_debug( "mqtt_enqueue id: %d, type=%d successful",
             client->mqtt_state.pending_msg_id, client->mqtt_state.pending_msg_type);
    if (client->mqtt_state.pending_msg_count > 0) {
        outbox_message_t msg = { 0 };
        msg.data = client->mqtt_state.outbound_message->data;
...
static int mqtt_message_receive(axk_mqtt_client_handle_t client, int read_poll_timeout_ms)
{
    ...
    if (client->mqtt_state.in_buffer_read_len == 0) {
        ...
        if (read_len < 0) {
            blog_error( "%s: transport_read() error: errno=%d", __func__, errno);
            goto err;
        }
        if (read_len == 0) {
            blog_info("%s: transport_read(): no data or EOF", __func__);
            return 0;
        }
        blog_debug( "%s: first byte: 0x%x", __func__, *buf);
        /*
        ...
        if (!mqtt_has_valid_msg_hdr(buf, read_len)) {
            blog_error( "%s: received a message with an invalid header=0x%x", __func__, *buf);
            goto err;
        }
        buf++;
    ...
    if ((client->mqtt_state.in_buffer_read_len == 1) ||
        ...
        do {
            ...
            if (read_len == 0) {
                blog_debug( "%s: transport_read(): no data or EOF", __func__);
                return 0;
            }
            blog_debug( "%s: read \"remaining length\" byte: 0x%x", __func__, *buf);
            buf++;
            client->mqtt_state.in_buffer_read_len++;
    ...
    blog_debug( "%s: total message length: %d (already read: %zu)", __func__, total_len, client->mqtt_state.in_buffer_read_len);
    ...
    if (client->mqtt_state.in_buffer_length < total_len) {
        if (mqtt_get_type(client->mqtt_state.in_buffer) == MQTT_MSG_TYPE_PUBLISH) {
            ...
            if (client->mqtt_state.in_buffer_read_len < fixed_header_len + 2) {
                ...
                blog_debug( "%s: read_len=%d", __func__, read_len);
                if (read_len < 0) {
                    blog_error( "%s: transport_read() error: errno=%d", __func__, errno);
                    goto err;
                } else if (read_len == 0) {
                ...
                buf += read_len;
                if (client->mqtt_state.in_buffer_read_len < fixed_header_len + 2) {
                    blog_debug( "%s: transport_read(): message reading left in progress :: total message length: %d (already read: %zu)",
            ...
            blog_debug( "%s: total len modified to %d as message longer than input buffer", __func__, total_len);
            if (client->mqtt_state.in_buffer_length < total_len) {
                blog_error( "%s: message is too big, insufficient buffer size", __func__);
                goto err;
            } else {
    ...
    blog_debug( "%s: transport_read():%zu %zu", __func__, client->mqtt_state.in_buffer_read_len, client->mqtt_state.message_length);
    return 1;
err:
    axk_mqtt_client_dispatch_transport_error(client);
    return -1;
...
static axk_err_t mqtt_process_receive(axk_mqtt_client_handle_t client)
{
    ...
    if (recv < 0) {
        blog_error( "%s: mqtt_message_receive() returned %d", __func__, recv);
        return AXK_FAIL;
    }
    if (recv == 0) {
        return AXK_OK;
    ...
    blog_debug( "msg_type=%d, msg_id=%d", msg_type, msg_id);

    switch (msg_type) {
    case MQTT_MSG_TYPE_SUBACK:
        if (is_valid_mqtt_msg(client, MQTT_MSG_TYPE_SUBSCRIBE, msg_id)) {
            blog_debug( "deliver_suback, message_length_read=%zu, message_length=%zu", client->mqtt_state.in_buffer_read_len, client->mqtt_state.message_length);
            if (deliver_suback(client) != AXK_OK) {
                blog_error( "Failed to deliver suback message id=%d", msg_id);
                return AXK_FAIL;
            }
        }
        break;
    case MQTT_MSG_TYPE_UNSUBACK:
        if (is_valid_mqtt_msg(client, MQTT_MSG_TYPE_UNSUBSCRIBE, msg_id)) {
            blog_debug( "UnSubscribe successful");
            client->event.event_id = MQTT_EVENT_UNSUBSCRIBED;
            axk_mqtt_dispatch_event_with_msgid(client);
        }
        break;
    case MQTT_MSG_TYPE_PUBLISH:
        blog_debug( "deliver_publish, message_length_read=%zu, message_length=%zu", client->mqtt_state.in_buffer_read_len, client->mqtt_state.message_length);
        if (deliver_publish(client) != AXK_OK) {
            blog_error( "Failed to deliver publish message id=%d", msg_id);
            return AXK_FAIL;
        }
        if (msg_qos == 1) {
        ...
        if (client->mqtt_state.outbound_message->length == 0) {
            blog_error( "Publish raxkonse message PUBACK or PUBREC cannot be created");
            return AXK_FAIL;
        }

        if (msg_qos == 1 || msg_qos == 2) {
            blog_debug( "Queue raxkonse QoS: %d", msg_qos);

            if (mqtt_write_data(client) != AXK_OK) {
                blog_error( "Error write qos msg repsonse, qos = %d", msg_qos);
                return AXK_FAIL;
            }
        }
        break;
    case MQTT_MSG_TYPE_PUBACK:
        if (is_valid_mqtt_msg(client, MQTT_MSG_TYPE_PUBLISH, msg_id)) {
            blog_debug( "received MQTT_MSG_TYPE_PUBACK, finish QoS1 publish");
            client->event.event_id = MQTT_EVENT_PUBLISHED;
            axk_mqtt_dispatch_event_with_msgid(client);
        }
        break;
    case MQTT_MSG_TYPE_PUBREC:
        blog_debug( "received MQTT_MSG_TYPE_PUBREC");
        client->mqtt_state.outbound_message = mqtt_msg_pubrel(&client->mqtt_state.mqtt_connection, msg_id);
        if (client->mqtt_state.outbound_message->length == 0) {
            blog_error( "Publish raxkonse message PUBREL cannot be created");
            return AXK_FAIL;
        }
    ...
    case MQTT_MSG_TYPE_PUBREL:
        blog_debug( "received MQTT_MSG_TYPE_PUBREL");
        client->mqtt_state.outbound_message = mqtt_msg_pubcomp(&client->mqtt_state.mqtt_connection, msg_id);
        if (client->mqtt_state.outbound_message->length == 0) {
            blog_error( "Publish raxkonse message PUBCOMP cannot be created");
            return AXK_FAIL;
        }
    ...
    case MQTT_MSG_TYPE_PUBCOMP:
        blog_debug( "received MQTT_MSG_TYPE_PUBCOMP");
        if (is_valid_mqtt_msg(client, MQTT_MSG_TYPE_PUBLISH, msg_id)) {
            blog_debug( "Receive MQTT_MSG_TYPE_PUBCOMP, finish QoS2 publish");
            client->event.event_id = MQTT_EVENT_PUBLISHED;
            axk_mqtt_dispatch_event_with_msgid(client);
        }
        break;
    case MQTT_MSG_TYPE_PINGRAXK:
        blog_debug( "MQTT_MSG_TYPE_PINGRAXK");
        client->wait_for_ping_raxk = false;
        break;
    }
...
static axk_err_t mqtt_resend_queued(axk_mqtt_client_handle_t client, outbox_item_handle_t item)
{
    ...
    if (mqtt_write_data(client) != AXK_OK) {
        blog_error( "Error to resend data ");
    ...
    if (client->mqtt_state.pending_msg_type == MQTT_MSG_TYPE_PUBLISH && client->mqtt_state.pending_publish_qos == 0) {
        ...
        if (outbox_delete_item(client->outbox, item) != AXK_OK) {
            blog_error( "Failed to remove queued qos0 message from the outbox");
        }
    }
    return AXK_OK;
}
...
static void mqtt_delete_expired_messages(axk_mqtt_client_handle_t client)
{
    ...
    while ((msg_id = outbox_delete_single_expired(client->outbox, platform_tick_get_ms(), OUTBOX_EXPIRED_TIMEOUT_MS)) > 0) {
        ...
        if (axk_mqtt_dispatch_event(client) != AXK_OK) {
            blog_error( "Failed to post event on deleting message id=%d", msg_id);
        }
        deleted_items ++;
    }
#else
...
static void axk_mqtt_task(void *pv)
{
    ...
    if (client->transport == NULL) {
        blog_error( "There are no transports valid, stop mqtt client, config scheme = %s", client->config->scheme);
        client->run = false;
    }
    //default port
    ...
    while (client->run) {
        ...
        case MQTT_STATE_INIT:
            ...
            if (client->transport == NULL) {
                blog_error( "There is no transport");
                client->run = false;
            }
#if MQTT_ENABLE_SSL
            ...
            if (axk_transport_connect(client->transport,
                ...
                blog_error( "Error transport connect");
                axk_mqtt_client_dispatch_transport_error(client);
                axk_mqtt_abort_connection(client);
                break;
            }
            blog_debug( "Transport connected to %s://%s:%d", client->config->scheme, client->config->host, client->config->port);
            if (axk_mqtt_connect(client, client->config->network_timeout_ms) != AXK_OK) {
                blog_error( "MQTT connect failed");
                axk_mqtt_abort_connection(client);
                break;
            }
        ...
        case MQTT_STATE_CONNECTED:
            ...
            if (client->connect_info.keepalive &&       // connect_info.keepalive=0 means that the keepslive is disabled
                ...
                if (client->wait_for_ping_raxk) {
                    blog_error( "No PING_RAXK, disconnected");
                    axk_mqtt_abort_connection(client);
                    client->wait_for_ping_raxk = false;
                    break;
                }
                if (axk_mqtt_client_ping(client) == AXK_FAIL) {
                    blog_error( "Can't send ping, disconnected");
                    axk_mqtt_abort_connection(client);
                    break;
                } else {
                    client->wait_for_ping_raxk = true;
                }
                blog_debug( "PING sent");
            }
            ...
            if (client->config->refresh_connection_after_ms &&
                ...
                blog_debug( "Refreshing the connection...");
                axk_mqtt_abort_connection(client);
                client->state = MQTT_STATE_INIT;
            }
        ...
        case MQTT_STATE_WAIT_RECONNECT:
            ...
            continue;
        default:
            blog_error( "MQTT client error, client is in an unrecoverable state.");
            break;
        }
        MQTT_API_UNLOCK(client);
        if (MQTT_STATE_CONNECTED == client->state) {
            if (axk_transport_poll_read(client->transport, MQTT_POLL_READ_TIMEOUT_MS) < 0) {
                blog_error( "Poll read error: %d, aborting connection", errno);
                axk_mqtt_abort_connection(client);
            }
        }
...
{
    if (!client) {
        blog_error( "Client was not initialized");
        return AXK_ERR_INVALID_ARG;
    }
    MQTT_API_LOCK(client);
    if (client->state != MQTT_STATE_INIT && client->state != MQTT_STATE_DISCONNECTED) {
        blog_error( "Client has started");
        MQTT_API_UNLOCK(client);
        return AXK_FAIL;
    }
    axk_err_t err = AXK_OK;
#if MQTT_CORE_SELECTION_ENABLED
    blog_debug( "Core selection enabled on %u", MQTT_TASK_CORE);
    if (xTaskCreatePinnedToCore(axk_mqtt_task, "mqtt_task", client->config->task_stack, client, client->config->task_prio, &client->task_handle, MQTT_TASK_CORE) != pdTRUE) {
        blog_error( "Error create mqtt task");
        err = AXK_FAIL;
    }
#else
    blog_debug( "Core selection disabled");
...
axk_err_t axk_mqtt_client_disconnect(axk_mqtt_client_handle_t client)
{
    ...
    blog_info( "Client asked to disconnect");
...
axk_err_t axk_mqtt_client_reconnect(axk_mqtt_client_handle_t client)
{
    ...
    blog_info( "Client force reconnect requested");
    if (client->state != MQTT_STATE_WAIT_RECONNECT) {
        blog_debug( "The client is not waiting for reconnection. Ignore the request");
        return AXK_FAIL;
    }
...
static axk_err_t send_disconnect_msg(axk_mqtt_client_handle_t client)
{
    ...
    if (client->mqtt_state.outbound_message->length == 0) {
        blog_error( "Disconnect message cannot be created");
        return AXK_FAIL;
    }
    if (mqtt_write_data(client) != AXK_OK) {
        blog_error( "Error sending disconnect message");
    }
    return AXK_OK;
}
...
axk_err_t axk_mqtt_client_stop(axk_mqtt_client_handle_t client)
{
    ...
    if (client->run) {
        ...
        if (running_task == client->task_handle) {
            MQTT_API_UNLOCK(client);
            blog_error( "Client cannot be stopped from MQTT task");
            return AXK_FAIL;
        }
...
static axk_err_t axk_mqtt_client_ping(axk_mqtt_client_handle_t client)
{
    ...
    if (client->mqtt_state.outbound_message->length == 0) {
        blog_error( "Ping message cannot be created");
        return AXK_FAIL;
    }

    if (mqtt_write_data(client) != AXK_OK) {
        blog_error( "Error sending ping");
        return AXK_FAIL;
    }
    blog_debug( "Sent PING successful");
    return AXK_OK;
}
...
int axk_mqtt_client_subscribe(axk_mqtt_client_handle_t client, const char *topic, int qos)
{
    ...
    MQTT_API_LOCK(client);
    if (client->state != MQTT_STATE_CONNECTED) {
        blog_error( "Client has not connected");
        MQTT_API_UNLOCK(client);
        return -1;
    }
    client->mqtt_state.outbound_message = mqtt_msg_subscribe(&client->mqtt_state.mqtt_connection,
                                          ...
                                          &client->mqtt_state.pending_msg_id);
    if (client->mqtt_state.outbound_message->length == 0) {
        blog_error( "Subscribe message cannot be created");
        MQTT_API_UNLOCK(client);
        return -1;
    }
    ...
    if (mqtt_write_data(client) != AXK_OK) {
        blog_error( "Error to subscribe topic=%s, qos=%d", topic, qos);
        MQTT_API_UNLOCK(client);
        return -1;
    }

    blog_debug( "Sent subscribe topic=%s, id: %d, type=%d successful", topic, client->mqtt_state.pending_msg_id, client->mqtt_state.pending_msg_type);
    MQTT_API_UNLOCK(client);
...
int axk_mqtt_client_unsubscribe(axk_mqtt_client_handle_t client, const char *topic)
{
    ...
    if (client->mqtt_state.outbound_message->length == 0) {
        MQTT_API_UNLOCK(client);
        blog_error( "Unubscribe message cannot be created");
        return -1;
    }
    blog_debug( "unsubscribe, topic\"%s\", id: %d", topic, client->mqtt_state.pending_msg_id);
    ...
    if (mqtt_write_data(client) != AXK_OK) {
        blog_error( "Error to unsubscribe topic=%s", topic);
        MQTT_API_UNLOCK(client);
        return -1;
    }

    blog_debug( "Sent Unsubscribe topic=%s, id: %d, successful", topic, client->mqtt_state.pending_msg_id);
    MQTT_API_UNLOCK(client);
    return client->mqtt_state.pending_msg_id;
}
        ...
        int len, int qos, int retain, bool store)
{
    ...
    if (publish_msg->length == 0) {
        blog_error( "Publish message cannot be created");
        return -1;
    }
...
int axk_mqtt_client_publish(axk_mqtt_client_handle_t client, const char *topic, const char *data, int len, int qos, int retain)
{
    ...
    if (client->state != MQTT_STATE_CONNECTED) {
        blog_info( "Publishing skipped: client is not connected");
        MQTT_API_UNLOCK(client);
        return -1;
    }
#endif
    ...
    if (client->state != MQTT_STATE_CONNECTED) {
        blog_debug( "Publish: client is not connected");
        if (qos > 0) {
            ret = pending_msg_id;
        }
    ...
    while (sending)  {
        ...
        if (remaining_len > 0) {
            ...
            blog_debug( "Sending fragmented message, remains to send %d bytes of %d", remaining_len, len);
...
cannot_publish:
    ...
    client->mqtt_state.outbound_message->fragmented_msg_total_length = 0;
    if (qos == 0) {
        blog_warn( "Publish: Losing qos0 data when client not connected");
    }
    MQTT_API_UNLOCK(client);
...
#ifdef MQTT_SUPPORTED_FEATURE_EVENT_LOOP
    if (client->config->event_handle) {
        blog_warn( "Registering event loop while event callback is not null, clearing callback");
...
#else
    blog_error( "Registering event handler while event loop not available in SDK version %s", SDK_VER);
    return AXK_FAIL;
#endif
}
